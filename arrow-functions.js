...
const evaluateScores = (a, b, c) => {
    ...
    if (highScore >= 90) {
        return `Excellent work! Top score: ${highScore}`;
    }
    if (highScore >= 70) {
        return `Good job! Top score: ${highScore}`;
    } else {
        return `Keep practicing! Top score: ${highScore}`;
    }
};
...
const calculateSalePrice = (name, price) => {
    ...
    return `Product: ${name} - Sale Price: $${newPrice};`
};
